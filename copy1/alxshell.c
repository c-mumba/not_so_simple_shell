...
char *read_user_input(void)
{
    ...
    printf("INSIDE read user input\n");
	user_input = (char *)malloc(10 * sizeof(char));
	printf("USEEERRRR INPUT = %s\n", user_input);
   /*
	if (user_input == NULL)
    return (NULL);
*/
...
printf("DEFFFUSEEERRRR INPUT = %s\n", user_input);
   /* free (user_input);
    printf("SEEERRRR INPUT = %s\n", user_input);*/
	return (user_input);
...
char *commandPath(char *command)
{
    ...
    while (path_token != NULL)
    {
        ...
        {
			printf("INSIDE COMMAND PATH11 = %s\n", command);
           /* free(command_path);*/
            return (command_path);
        }
        else
...
int execute(char **list_tokens, char **LT, char **en)
{
	...
	printf("INSIDE EXECUTION\n");
printf("FF = %s\n",list_tokens[0]);
/*	while (list_tokens[k])
	{
printf("x[%d] = %s and first = %s\n", k,  list_tokens[k], first);
k++;
	}
	 printf("x[%d] = %s and first = %s\n", k,  list_tokens[k], first);
*/
...
printf("USER INPUT = %s\n", user_input);
        ...
        if (user_input)
        {
            ...
            printf("BBBBB_tokens[0] = %s\n", list_tokenss[0]);
			if (builtins_list(list_tokenss,envp) >= 0)
            {
				printf("BUILTINS EXECUTED!\n");
